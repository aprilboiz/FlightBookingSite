...
func (f *flightRepository) GetAll() ([]*models.Flight, error) {
	...
	result := f.db.
		...
		Find(&flights)
	if result.Error != nil {
		return nil, exceptions.InternalError("failed to get all flights", result.Error)
	}
	return flights, nil
}
...
func (f *flightRepository) GetByCode(code string) (*models.Flight, error) {
	...
	if result.Error != nil {
		if errors.Is(result.Error, gorm.ErrRecordNotFound) {
			return nil, exceptions.NotFoundError("flight", code)
		}
		return nil, exceptions.InternalError("failed to get flight by code", result.Error)
	}
	return &flight, nil
}
...
func (f *flightRepository) Create(flight *models.Flight) (*models.Flight, error) {
	...
	if result.Error != nil {
		return nil, exceptions.InternalError("failed to create flight", result.Error)
	}
	return flight, nil
}
...
func (f *flightRepository) Update(flight *models.Flight) (*models.Flight, error) {
	...
	if result.Error != nil {
		return nil, exceptions.InternalError("failed to update flight", result.Error)
	}
	return flight, nil
}
...
func (f *flightRepository) Delete(flight *models.Flight) error {
	...
	if result.Error != nil {
		return exceptions.InternalError("failed to delete flight", result.Error)
	}
	return nil
}
...
func (f *flightRepository) CreateIntermediateStops(stops []*models.IntermediateStop) ([]*models.IntermediateStop, error) {
	...
	if result.Error != nil {
		return nil, exceptions.InternalError("failed to create intermediate stops", result.Error)
	}
	return stops, nil
}
...
func (f *flightRepository) GetAvailableSeats(flightCode string) ([]*models.Seat, error) {
	...
	result := f.db.
		...
		Find(&seats)
	if result.Error != nil {
		return nil, exceptions.InternalError("failed to get available seats", result.Error)
	}
	return seats, nil
}
...
func (f *flightRepository) DeleteIntermediateStops(flightID uint) error {
	...
	if result.Error != nil {
		return exceptions.InternalError("failed to delete intermediate stops", result.Error)
	}
	return nil
}
...
func (f *flightRepository) GetFlightsByDateRange(startDate, endDate time.Time) ([]*models.Flight, error) {
	var flights []*models.Flight
	result := f.db.
		Preload("Plane").
		Preload("DepartureAirport").
		...
		Find(&flights)
	if result.Error != nil {
		return nil, exceptions.InternalError("failed to get flights by date range", result.Error)
	}
	return flights, nil
}
